...
Box.prototype.act = function(step, level) {
  var newPos = this.pos.plus(this.speed.times(step));
  if (!level.obstacleAt(newPos, this.size))
    this.pos = newPos;
  else if (this.repeatPos)
...
Missle.prototype.act = function(step, level) {
  var newPos = this.pos.plus(this.speed.times(step));
  if (!level.obstacleAt(newPos, this.size))
    this.pos = newPos;
  else if (this.repeatPos)
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 0;
	alert('DON\'T STEP IN THE ACID...DUMBASS');
  }
  else if (type == "missle" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 0;
	alert('Avoid the Missles');
  }
  else if (type == "box" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 2;
	alert('Avoid the birds');
  }  
  else if (type == "coin") {
